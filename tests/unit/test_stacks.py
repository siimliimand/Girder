...
from girder.orchestrator.task_engine import VERIFY_XML, verify_cmd
from girder.stacks import STACK_REGISTRY, UnknownStackError, get_stack
from girder.stacks.golang import GoPlugin
from girder.stacks.node import NodePlugin
from girder.stacks.python import PythonPlugin
...
def test_load_settings_rejects_unknown_stack(tmp_path: Path) -> None:
    ...
    with pytest.raises(
        ValueError, match=r"project.stack 'cobol-85' is not a registered stack"
    ):
        load_settings(toml)
...
def test_python_test_command_byte_identical() -> None:
    for pybin in (None, "python3", "/usr/bin/python3.12"):
        ...
        # Same argv the engine used to build inline (task_engine.verify_cmd).
        assert PythonPlugin().test_command("python3") == verify_cmd("python3")
