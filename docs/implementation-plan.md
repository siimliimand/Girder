...
| R18 | §4.3 `agent_events`/`token_usage` schema | Live schema (migrations 004/008) makes `attempt_id` nullable and adds `run_id` | Spec-generation and pre-attempt spend events precede attempt creation, and run-level budget accounting needs a run-scoped key; additive drift, FK intent preserved |
| R19 | §6.4 literal cache-mount paths (`/root/.cache/pip`, `/root/.npm`, `/usr/local/cargo/registry`) | Implemented as `/cache/{pip,npm,cargo}` with image-set `PIP_CACHE_DIR`/`npm_config_cache`/`CARGO_HOME` (documented in sandbox/task-engine code) | The non-root in-guest user cannot traverse `/root` with `CAP_DAC_OVERRIDE` dropped; same RO guarantee |
| R20 | Phase 4 "suite after EACH merge" | The integrator skips the per-merge suite re-run ONLY when a conflict-resolution commit already passed the full suite at that exact SHA (fail-closed exact-hash memoization, documented in orchestrator/integrator.py); every other merge runs the full suite | Memoizing on the exact SHA preserves the rail's guarantee while avoiding a provably redundant re-run |
| R21 | §6.9 "spec mounted read-only into containers" | Implemented via the ScopeGuard write-hold on `openspec/**` (protected_read_paths) plus the Layer-3 orchestrator-side protected-path audit, not a literal RO volume mount; tamper evidence (SHA-256 anchor) unchanged | Agent writes to spec paths are held violations by construction, which meets the intent without a mount the podman invocation cannot express |
| R22 | §5.1 has no `paused` state | Pause is implemented as a `runs.paused` flag (migration 010) honored at the task boundary by the pump, not a new FSM state | Avoids adding a state that every existing edge would need to reach |
| R23 | §6.13 `notify(level, title, body_builder, run_id)` | Implemented as `notify(level, title, body, run_id)` taking a rendered body; redaction and `notifications_log` semantics unchanged | A pre-rendered body keeps the notification surface serializable and testable with identical guarantees |
| R24 | §Phase 3 task 3: "poll every 30s" | The literal polling primitive (`ci.py wait_for_settled`) is retained for tests; the production delivery engine derives poll deadlines from persisted state transitions, which supersedes the fixed interval | Derived deadlines are crash-resume-safe and restart-safe; a fixed interval is not |
| R25 | §8.4 amendment resolution | NEW capability (fix pass): `spec_amendments` gains `scope_globs_json` (migration 013); an Approve resolution may carry replacement/extra task `scope_globs`, unioned onto the task on approval. Also, `resolve_amendment` accepts a run still in `active` when a pending amendment exists (crash between amendment-row creation and the run transition left the pair unresolvable — §8.7 reconciliation requirement) | Without scope union, an approved amendment requiring out-of-original-scope writes would immediately die as an integrity violation |
| R26 | §5.1 run FSM | Fix pass additions: explicit `BUDGET_EXHAUSTED → ACTIVE` edge — raising `budget_cap_usd` resumes a budget-frozen run (§8.3 ceilings are per-cap); the pump handles `escalated` runs for the R14 operator-abort exit | The R14 exit was declared but unreachable without a pump handler; a frozen run with a raised cap must not stay deadlocked |
| R27 | §6.1 "at least one model role per tier" | Fix pass: enforced at `load_settings` — an empty `[[models.roles]]` registry is a hard configuration error; the dataclass itself stays permissive for internal/test construction | Config errors must fail loudly at load, not mid-run |
| R28 | §6.6 command denylist | Fix pass: matching is argv-aware — shlex-normalized tokens defeat quote/backslash obfuscation, and `git … push` is detected as separate tokens, not the literal substring `git push` | Substring matching is trivially bypassed by quoting and token splitting |

**Fix pass 2026-09-09:** four scope/denylist hardenings shipped alongside R25–R28 (no new scenario rows):
- `run_command` checks protected paths against every multi-token argument, not just the first path-like token.
- `strict_read_scope` is now honored for `run_command` reads, matching §8.5's literal behavior for file tools.
- Spend is rolled up per task in addition to per run, so per-task budget visibility matches the console's run-level view.
- The scheduler reports transitive demotions (a demoted task cascading to its dependents) instead of silently reordering.
