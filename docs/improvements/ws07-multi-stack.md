...
- **DONE (2026-09-11, follow-up): config-driven tool-exec interpreter.** The tool snippets' `python3 -c` execs (write_file/apply_patch/edit_file/list_directory) and the python stack's outline command no longer hardcode `python3` — `ToolRegistry`/`AgentRuntime` take `python_bin` threaded from `settings.sandbox.python_bin` (the same knob baseline/verify already used), and `symbol_outline_command(path, python_bin=None)` mirrors `test_command`'s signature. Runner images with a non-default interpreter path are now a config change; `run_tests` still execs bare `pytest` deliberately (an image contract, like `git`/`rg`/`sed`).
