...
# ------------------------------------------------- WS-05: automation webhooks
#
# Additive section (wave 1): inbound GitHub webhook receiver (WP 9.1) and the
# Slack slash-command / interactive endpoint (WP 9.2). Both authenticate via
# HMAC instead of the console's session model; WS-10's API-key middleware
# should exempt these paths (webhooks authenticate via their own signatures).
...
async def _slack_create_run(
    ...
    db: Database = app.state.db
    settings: Settings = app.state.settings
    window = await _review_window_state(db, settings, project)
    if window["exceeded"]:
        return _review_window_message(settings.autonomy.t1_review_window)
    run = await repo.create_run(
