...
class GitHubClient:
    ...
    #: Built-in PR template (WS-05 / WP 9.3) used when ``[github] pr_template``
    #: is not configured.
    BUILTIN_PR_TEMPLATE = """\
## Summary
...
"""
    ...
    def pr_body(
        ...
        *,
        ...
        """PR body for a run (§ Phase 3 task 2; WP 9.3 template integration).

        Legacy plain-text format when only the frozen-spec metadata is known
        (no ``run_id`` — the delivery pump's current call shape). With a
        ``run_id``, the configured ``[github] pr_template`` (Jinja2) is
        rendered — falling back to ``BUILTIN_PR_TEMPLATE`` — with the run
        context: intent, per-task summary, cost, attempts, and a console link.
        """
        if run_id is None:
            lines = [
                "## Intent",
        ...
        context = {
            "intent": intent.strip(),
            "tasks": tasks
            or [{"title": t, "task_type": "", "status": ""} for t in task_lines],
            "criteria": criteria,
